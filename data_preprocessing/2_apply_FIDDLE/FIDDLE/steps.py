...
def process_time_dependent(df_data_time_series, args):
    ...
    df_time_series = joblib.load(output_dir + 'df_time_series.joblib')
    dtypes_time_series = joblib.load(output_dir + 'dtypes_time_series.joblib')
...
#    X_all = sparse.load_npz(dir_path + 'X_all.npz')
#    X_all_feature_names = json.load(open(dir_path + 'X_all.feature_names.json', 'r'))
#    X_discretization_bins = json.load(open(dir_path + 'X_all.discretization.json', 'r'))
    print('Time elapsed: %f seconds' % (time.time() - start_time))
...
def transform_time_series_table(df_in, args):
    ...
    # freq_file = open('freq.p', 'wb')
    # var_file = open('var.p', 'wb')
    # variables_num_freq = pickle.load(freq_file)
    
    # pickle.dump(variables_num_freq,freq_file)
    ...
    if args.parallel:
        ...
        #     out[i] = func_encode_single_time_series(i, g.set_index(ID_col), variables, variables_num_freq, args.T, args.dt, args.stats_functions)
        #     num+=1
        #     print(num)
        out = dict(
    ...
    # premature =  open('pre.p', 'wb')
    # pickle.dump(out,premature)
    # premature.close()
    df_in = 0    
...
#        freq_file = open('dis_bin.p', 'rb')
#        discretization_bins = pickle.load(freq_file)
#        freq_file.close()
