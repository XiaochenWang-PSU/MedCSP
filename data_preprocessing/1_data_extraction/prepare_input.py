...
def resolve_duplicates_discrete(data):
    ...
    # print(len(df['t'].isna()))
    # print(df.columns)
    # print(df.duplicated(subset=['ID', 't', 'variable_name']).value_counts())
    m_dups = df.duplicated(subset=['ID', 't', 'variable_name'], keep=False)
    # print(m_dups.value_counts())
    dups = df[m_dups]
...
def main():
    ...
    outcome, T, dt = "pretrain", 48.0, 1.0
    ...
    print(data.keys())
    data["LABEVENTS"] = data.pop("combined_lab").drop_duplicates()
    data["MICROBIOLOGYEVENTS"] = data.pop("combined_bio").drop_duplicates()
    data["DATETIMEEVENTS"] = data.pop("combined_dte").drop_duplicates()
    data["OUTPUTEVENTS"] = data.pop("combined_opt").drop_duplicates()
    data["CHARTEVENTS"] = data.pop("combined_chr").drop_duplicates()
    data["INPUTEVENTS_MV"] = data.pop("combined_ipt").drop_duplicates()
    data["PROCEDUREEVENTS_MV"] = data.pop("combined_pcd").drop_duplicates()
   
    print('start trans')
    tables = list(data.keys())
    for i in tables:
        print(i)
        data[i]['variable_name'] = data[i]['variable_name'].astype(str)
    ...
    df.to_pickle(data_path + 'features/outcome={},T={},dt={}/processed_data.p'.format(outcome, T, dt))
    # data.to_pickle(data_path + 'features/outcome={},T={},dt={}/processed_data.p'.format(outcome, T, dt))
    # # Combine all DataFrames into one
    print(data['TIME_INVARIANT'].duplicated().sum())
    import pickle
   
    with open(data_path + 'features/outcome={},T={},dt={}/processed_data.p'.format(outcome, T, dt), 'wb') as handle:
    ...
    # # Save
    # print(data.keys())
    df_data = df_data.drop_duplicates(subset = ['ID', 't', 'variable_name'])
