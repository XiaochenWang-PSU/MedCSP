...
def main():
    ...
    extract_events('combined_lab', 'CHARTTIME', ['VALUE', 'FLAG'])
    extract_events('combined_bio', 'CHARTTIME')
    extract_events('combined_dte', 'CHARTTIME', ['VALUE'])
    extract_events('combined_opt', 'CHARTTIME', ['VALUE'])
    extract_events('combined_chr', 'CHARTTIME', ['VALUE'], 2000000)
...
def merge_items_table():
    ...
    d_items = pd.read_csv(mimic3_path + 'combined_itm.csv')
    # d_items = d_items[d_items['DBSOURCE'].isin(['metavision', 'hospital'])]
    d_labitems = pd.read_csv(mimic3_path + 'combined_ltm.csv')
...
def extract_invariant():
    ...
    patients = pd.read_csv(
        mimic3_path + 'combined_pat.csv', 
        parse_dates=['DOB'], usecols=['SUBJECT_ID', 'DOB', 'GENDER', 'ANCHOR_AGE']
    )
    
    admissions = pd.read_csv(
        mimic3_path + 'combined_adm.csv', 
    ...
    examples = examples[['SUBJECT_ID', 'HADM_ID', 'ICUSTAY_ID', 'FIRST_CAREUNIT',  'INTIME']]
    ...
    invariant['AGE'] = invariant[['INTIME', 'DOB']].apply(lambda x: (x.INTIME.to_pydatetime() - x.DOB.to_pydatetime()).total_seconds()/ (3600 * 24 * 365) if x.DOB else x.ANCHOR_AGE, axis=1) 
...
def _read_events(fname, t_cols, chunksize):
    ...
    with tqdm(desc=fname, total=(n_rows//chunksize+1)) as pbar:
        for df in pd.read_csv(mimic3_path + '{}.csv'.format(fname), parse_dates=t_cols, chunksize=chunksize):#, usecols = ['VALUENUM', 'STORETIME', 'WARNING', 'ITEMID', 'VALUE','CHARTTIME', 'ICUSTAY_ID', 'VALUEUOM']):
            pbar.update()
            yield df
...
def _process_chunk(df, fname):
    ...
    if 'ICUSTAY_ID' not in df.columns:
        df_icu = pd.read_csv(mimic3_path + 'combined_icu.csv', parse_dates=['INTIME', 'OUTTIME']).sort_values(by='SUBJECT_ID').reset_index(drop=True)
        df = pd.merge(df_icu[['SUBJECT_ID', 'HADM_ID', 'ICUSTAY_ID']], df, on=['SUBJECT_ID', 'HADM_ID'], how='inner')
        df = df.drop(columns=['SUBJECT_ID', 'HADM_ID'])
    
    if fname.lower() == 'combined_bio':
    ...
    if fname.lower() == 'combined_opt':
        # Remove error entries
        pass
        # df = df.loc[df.ISERROR.isnull()]
    
    if fname.lower() == 'combined_chr':
        # Remove error entries
...
def extract_events(fname, t_col, val_cols=[], chunksize=500000):
    ...
    for df_chunk in _read_events(fname, [t_col], chunksize):
        ...
        # Calculate t
        df_chunk[t_col] = pd.to_datetime(df_chunk[t_col])
        df_chunk['INTIME'] = pd.to_datetime(df_chunk['INTIME'])
